...
const DCCompetenciaCombinada = () => {
  ...
  const handleTerminarFaseClasif = async () => {
    ...
    if (noCompletados.length > 0) {
      ...
      Alert.alert("Fase Clasificatoria Terminada", "Se han generado los resultados de la siguiente fase");
      ...
      case "detalle":
        return (
          <View style={styles.faseContainer}>
            ...
            <Text style={styles.value}>Combinada
            </Text>
          </View>
        );
      case "deportistas":
            ...
            (
                      <View style={styles.resItemContainer}>
                        <View style={styles.resItemData}>
                          ...
                          {/* Mostrar formato de Tops y Zonas */}
                          <Text style={styles.label}>
                            {item.totalTops}T {item.totalZonas}HZ {item.totalZonasL}LZ {item.intentosTops} {item.intentosZonas} {item.intentosZonasL} ({item.puntajeCombinadaBloque?.toFixed(2)})
                          </Text>
                        </View>
                        <View style={styles.resBtnContainer}>
                          <TouchableOpacity
