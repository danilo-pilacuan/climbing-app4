...
const renderResultadoItem = ({ item }) => (
  <View style={styles.singleAthleteItem}>
    {item && (
      <View style={styles.athleteRowLayout}>
        <View style={styles.mainInfoSection}>
          <Text style={styles.athleteFullName}>{item.deportista.nombresDep} {item.deportista.apellidosDep}</Text>
          
          <View style={styles.resultsSection}>
            {item.registroEditadoT1 && (
              <View style={styles.timeDataBlock}>
                <Text style={styles.timeLabelText}>Tiempo 1:</Text>
                <Text style={[
                  styles.timeValueText,
                  item.salidaFalse && styles.falseStartStyle,
                  item.fallRegistro1 && styles.fallIndicator
                ]}>
                  {item.salidaFalse ? "FLS" : (item.fallRegistro1 ? "FALL" : item.tiempo1)}
                </Text>
              </View>
            )}
            ...
            {item.registroEditadoT2 && (
              <View style={styles.timeDataBlock}>
                <Text style={styles.timeLabelText}>Tiempo 2:</Text>
                <Text style={[
                  styles.timeValueText,
                  item.salidaFalse && styles.falseStartStyle,
                  item.fallRegistro2 && styles.fallIndicator
                ]}>
                  {item.salidaFalse ? "FLS" : (item.fallRegistro2 ? "FALL" : item.tiempo2)}
                </Text>
              </View>
            )}
          </View>
  ...
  const renderResultadoVSItem = ({ item }) => (
    <View style={styles.resVSItem}>
      {/* Título de la ronda */}
      {(item.registrosVS[0] ? item.registrosVS[0].etapa == 5 : false) && (
        <Text style={styles.roundTitle}>TERCER LUGAR</Text>
      )}
      {(item.registrosVS[0] ? item.registrosVS[0].etapa == 6 : false) && (
        <Text style={styles.roundTitle}>FINAL</Text>
      )}
      
      {item && (
        <View style={styles.matchupContainer}>
          {/* Fila de enfrentamiento con ambos deportistas */}
          <View style={styles.horizontalMatchup}>
            {/* Deportista 1 */}
            {item.registrosVS[0] && (
              <View style={[
                styles.athleteContainer,
                item.registrosVS[0].registroEditadoT1 && !item.registrosVS[0].fallRegistro1 && 
                item.registrosVS[1]?.registroEditadoT1 && 
                item.registrosVS[0].tiempo1 < item.registrosVS[1].tiempo1 && 
                styles.winnerContainer
              ]}>
                <Text style={styles.athleteNumber}>Deportista 1</Text>
                
                <View style={styles.athleteInfo}>
                  
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Nombre:</Text>
                    <Text style={styles.infoValue}>{item.registrosVS[0].deportista.nombresDep}</Text>
                  </View>
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Apellido:</Text>
                    <Text style={styles.infoValue}>{item.registrosVS[0].deportista.apellidosDep}</Text>
                  </View>
            ...
            {item.registrosVS[1] && (
              <View style={[
                styles.athleteContainer,
                item.registrosVS[1].registroEditadoT1 && !item.registrosVS[1].fallRegistro1 && 
                item.registrosVS[0]?.registroEditadoT1 && 
                item.registrosVS[1].tiempo1 < item.registrosVS[0].tiempo1 && 
                styles.winnerContainer
              ]}>
                <Text style={styles.athleteNumber}>Deportista 2</Text>
                
                <View style={styles.athleteInfo}>
                  
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Nombre:</Text>
                    <Text style={styles.infoValue}>{item.registrosVS[1].deportista.nombresDep}</Text>
                  </View>
                  <View style={styles.infoRow}>
                    <Text style={styles.infoLabel}>Apellido:</Text>
                    <Text style={styles.infoValue}>{item.registrosVS[1].deportista.apellidosDep}</Text>
                  </View>
