...
const DCEditarRegistroResultado = () => {
  ...
  const [topSeleccionado, setTopSeleccionado] = useState({ maxEscala1: false, maxEscala2: false });
  const [competencia, setCompetencia] = useState();

  const fetchRegistroResultado = async () => {
    try {
      const response = await api.get(`/api/RegistroResultado/${idRegistroResultado}`);
      console.log("Registro de resultado: 😈😈😈😈😈", JSON.stringify(response.data));
  ...
  const handleToggleTop = (key) => {
    ...
    if (!topSeleccionado[key]) {
      ...
      setRegistroResultado((prev) => ({ ...prev, [key]: registroResultado.maxPresas.toString() }));
    }
  };
  ...
  const handleGuardarCambios = async () => {
    try {
        console.log("Guardando cambios en registro de resultado: 👽👽🤢🤢", JSON.stringify(registroResultado));
      registroResultado.registroCompleto = true;

      const bodyUpdate={
        "idRegistroResultado": registroResultado.idRegistroResultado,
        "MaxEscala1": registroResultado.maxEscala1.toString(),
        "puntajeCombinadaVia": registroResultado.puntajeCombinadaVia.toString()
      }

      console.log("Body Update: 👽👽🐟🐟🐟", JSON.stringify(bodyUpdate));

      await api.put(`/api/RegistroResultado/updateviasCombinada`,bodyUpdate );
      Alert.alert('Éxito', 'Registro actualizado correctamente');
      navigation.goBack();
      if(competencia.idMod==3)
      {

          navigation.navigate('DCCompetenciaVias', { idCom: idCom, regUpdated: true });
      }
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Editar Registro de Resultado Vias Combinada</Text>
    { competencia && <Text style={styles.title}>{competencia.nombreCom}</Text>}
      ...
      {/* Aquí adaptamos la lógica de edición para utilizar los campos de Intento y MaxEscala */}
        ...
        <View style={styles.tableContainer}>
          ...
          <View style={[styles.row, styles.headerRow]}>
            ...
            {[1,2].map((num) => (
              <Text key={num} style={styles.headerCell}>{num==1 && "Presa"} {num==1 && "Puntaje"}</Text>
            ))}
          </View>
          ...
          <View style={styles.row}>
            <Text style={styles.labelCell}>Max Escala</Text>
              ...
              <TextInput
                ...
                style={styles.inputCell}
                keyboardType="numeric"
                value={registroResultado[`puntajeCombinadaVia`]?.toString() || ''}
                onChangeText={(value) => handleInputChange(`puntajeCombinadaVia`, value)}
                editable={!topSeleccionado[`puntajeCombinadaVia`]}
              />
